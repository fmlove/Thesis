...
LAMP1_mov_dot = ggplot(per_seg.per_cell[per_seg.per_cell$direction != "static",], aes(x = cond, y = count, fill = cond)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell[per_seg.per_cell$direction != "static",]$count), 25, ceiling), size = 3.5)
...
LAMP1_trace_dot = ggplot(traces.df, aes(x = cond, y = traces, fill = cond)) + 
  ...
  d_format + report_theme + theme(legend.position = "none") + 
  stat_compare_means(label.y = plyr::round_any(max(traces.df$traces), 10, ceiling), size = 3.5)
...
velocity_violin = ggplot(per_seg[(abs(per_seg$velocity) < 1000) & per_seg$direction!= "static",], aes(x = cond, fill = cond, y = abs(velocity), colour = cond)) + 
  ...
  d_format + report_theme + theme(legend.position = "none") + 
  stat_compare_means(label = 'p.signif', comparisons = list(c("Control", "WT"), c("Control", "MUT")))
...
LAMP1_mean_run_length = ggplot(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',], aes(x = cond, y = mean_dist, fill = cond, color = cond)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_dist), 30, ceiling), size = 3.5)
...
test <- ant %>% group_by(direction) %>% select(mean_speed, cond)
...
LAMP1_mean_run_velocity = ggplot(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',], aes(x = cond, y = mean_speed)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_speed), 12, ceiling), size = 3.5) +
...
## Combined figure
```{r LAMP1_full_figure, fig.width = 16, fig.height = 4}
...
```{r}
# GFP_example = read.bitmap("C:\\Users\\fl299\\PhD\\Presentations\\Third\ year\ seminar\\Ptdn_LAMP1_representative_kymographs\\CNC32_GFP_C2.png")
# WT_example = read.bitmap("C:\\Users\\fl299\\PhD\\Presentations\\Third\ year\ seminar\\Ptdn_LAMP1_representative_kymographs\\CNC32_WT_C8.png")
# Mut_example = read.bitmap("C:\\Users\\fl299\\PhD\\Presentations\\Third\ year\ seminar\\Ptdn_LAMP1_representative_kymographs\\CNC32_MUT_C4.png")
# 
# GFP_LAMP1_example.gg = image_as_ggplot(GFP_example) + labs(title = "    Control")
# WT_LAMP1_example.gg = image_as_ggplot(WT_example) + labs(title = "    WT Protrudin")
# Mut_LAMP1_example.gg = image_as_ggplot(Mut_example) + labs(title = "    Active Protrudin")
LAMP1_kymo_panel = image_as_ggplot(read.bitmap('C:\\Users\\fl299\\PhD\\LAMP1_kymo_panel.png'))
```

Patchwork
```{r LAMP1_full_figure, fig.width = 16, fig.height = 8, fig.path = "figures/full/"}
  LAMP1_kymo_panel/
...
```{r Karolinska_LAMP1, fig.width = 16, fig.height = 9, eval = FALSE}
