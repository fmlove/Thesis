...
```{r read_CSVs}
CNC78 = read.csv("C:\\Users\\fl299\\PhD\\Data\\Mine\\Spinning\ Disc\\300621_cnc78_dendrites\\max_projections\\GFP\\dendrite_sections\\cell_counter\\cell_counter_counts.csv", stringsAsFactors = F)
CNC79 = read.csv("C:\\Users\\fl299\\PhD\\Data\\Mine\\Spinning\ Disc\\070721_cnc79_dendrites\\max_projections\\GFP\\dendrite_sections\\cell_counter\\cell_counter_counts.csv", stringsAsFactors = F)
CNC80 = read.csv("C:\\Users\\fl299\\PhD\\Data\\Mine\\Spinning\ Disc\\140721_cnc80_dendrites\\max_projections\\GFP\\dendrite_sections\\cell_counter\\cell_counter_counts.csv", stringsAsFactors = F)
CNC79_DIV21 = read.csv("C:\\Users\\fl299\\PhD\\Data\\Mine\\Spinning\ Disc\\130721_cnc79_DIV21_dendrites\\max_projections\\GFP\\dendrite_sections\\blind\\cell_counter\\cell_counter_counts.csv", stringsAsFactors = F)
```
...
spine_counts.normalised.melt$variable = factor(spine_counts.normalised.melt$variable, levels = c('m_norm', 't_norm', 's_norm', 'f_norm'), labels = c('mushroom', 'thin', 'stubby', 'filopodia'))
...
```{r DIV15_total_mCh_stat, fig.width=8, fig.height=6}
ggplot(spine_counts[spine_counts$age == 'DIV15',], aes(x = cond, y = total, group = cond, fill = cond, colour = cond)) + 
  ...
  labs(x = 'Condition', y = 'Number of spines in 20μm section', title = 'Dendritic spines at DIV15') +
...
ggplot(spine_counts.melt[spine_counts.melt$age == 'DIV15',], aes(x = cond, y = value, group = cond, fill = cond, colour = cond)) + 
  ...
  labs(x = 'Condition', y = 'Number of spines in 20μm section', title = 'Dendritic spines at DIV15 by type') +
  d_format + report_theme

```

Normalised to total number of spines on each cell
```{r DIV15_type_norm, fig.width=8, fig.height=6}
ggplot(spine_counts.normalised.melt[spine_counts.normalised.melt$age == 'DIV15',], aes(x = cond, y = value, group = cond, fill = cond, colour = cond)) + 
  geom_dotplot(binaxis = "y", stackdir = "center", position = "dodge", dotsize = 0.7) + geom_boxplot(fill = NA, colour = 'black', outlier.colour = NA) + 
  facet_wrap(~variable) + 
  stat_compare_means(label = 'p.signif', ref.group = 'Control') + 
  labs(x = 'Condition', y = 'Number of spines/total spines in 20μm section', title = 'Normalised spines per cell at DIV15') +
  d_format + report_theme
```

Another way of looking at percentages
```{r bar_fill_DIV15, fig.width=8, fig.height=6}
ggplot(spine_counts.melt[spine_counts.melt$age == 'DIV15',], aes(x = cell_id, y = value, fill = variable)) + 
...
### Total number of spines
Stats with respect to mCherry control...
```{r DIV21_total_mCh_stat, fig.width=8, fig.height=6}
ggplot(spine_counts[spine_counts$age == 'DIV21',], aes(x = cond, y = total, group = cond, fill = cond, colour = cond)) + 
  geom_dotplot(binaxis = "y", stackdir = "center", position = "dodge", dotsize = 0.7) + 
  geom_boxplot(fill = NA, colour = 'black', outlier.colour = NA) + 
  stat_compare_means(label = 'p.signif', ref.group = 'Control', hide.ns = F) + 
  labs(x = 'Condition', y = 'Number of spines in 20μm section', title = 'Dendritic spines at DIV21') +
  d_format + report_theme
```
...and the same again but with WT protrudin as the reference group
...
ggplot(spine_counts.melt[spine_counts.melt$age == 'DIV21',], aes(x = cond, y = value, group = cond, fill = cond, colour = cond)) + 
  ...
  facet_wrap(~variable) + 
  stat_compare_means(label = 'p.signif', ref.group = 'Control') + 
  labs(x = 'Condition', y = 'Number of spines in 20μm section', title = 'Dendritic spines at DIV21 by type') +
  d_format + report_theme

```

Normalised to total number of spines on each cell
```{r DIV21_type_norm, fig.width=8, fig.height=6}
ggplot(spine_counts.normalised.melt[spine_counts.normalised.melt$age == 'DIV21',], aes(x = cond, y = value, group = cond, fill = cond, colour = cond)) + 
  geom_dotplot(binaxis = "y", stackdir = "center", position = "dodge", dotsize = 0.7) + geom_boxplot(fill = NA, colour = 'black', outlier.colour = NA) + 
  facet_wrap(~variable) + 
  stat_compare_means(label = 'p.signif', ref.group = 'Control') + 
  labs(x = 'Condition', y = 'Number of spines/total spines in 20μm section', title = 'Normalised spines per cell at DIV15') +
...
ggplot(spine_counts.melt[spine_counts.melt$age == 'DIV21',], aes(x = cell_id, y = value, fill = variable)) + 
  geom_bar(position="fill", stat="identity") + 
  facet_wrap(~cond, scales = "free_x") + 
