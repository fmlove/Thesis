...
trace_plot = ggplot(per_cell.df, aes(x = condition, y = traces)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_speed), 160, ceiling), size = p_fs)
...
GS_mov_dot = ggplot(per_seg.per_cell[per_seg.per_cell$direction != "static",], aes(x = condition, y = count, fill = condition)) + 
  ...
  d_format + report_theme + 
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_speed), 225, ceiling), size = p_fs)
...
GS_mean_run_length = ggplot(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',], aes(x = condition, y = mean_dist)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_speed), 16, ceiling), size = p_fs)
...
GS_mean_run_velocity = ggplot(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',], aes(x = condition, y = mean_speed)) + 
  ...
  stat_compare_means(label.y = plyr::round_any(max(per_seg.per_cell.means[per_seg.per_cell.means$direction!= 'static',]$mean_speed), 8.5, ceiling), size = p_fs) +
...
```{r St3Gal5_kymo_full_figure, fig.width = 16, fig.height = 8, fig.path = '../figures/full/'}
