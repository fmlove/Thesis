...
Neurons were co-transfected with mCherry or an mCherry-tagged protrudin construct and EGFP/Emerald.  Cells were imaged as Z-stacks at DIV15, either on the spinning disc at 100x (first experiment), or confocal at 63x (replicate) (see \@ref(METHODS) for microscope details).  Image stacks from the green channel were converted to maximum intensity projections, and a custom ImageJ macro was used to select and crop 20 \textmu{}m sections from a dendrite.  These images were then blinded, and the Cell Counter Fiji plugin was used to count and classify spines.  For spine length, the Fiji segmented line tool was used to measure along each spine.
...
```{r spine-type-full-figure, echo = F,  fig.width = 16, fig.height = 22, fig.path = '../figures/full/', fig.cap="spine-type-cap", fig.scap="Protrudin effect on dendritic spine morphology"}
...
In addition to protrusions, we noted that protrudin overexpression also induces the formation of large, ring-shaped structures in the perinuclear region of HeLa cells.  These are observed occasionally in neurons expressing protrudin, but not to the same extent as in HeLa cells (supplementary figure \@ref(fig:suppl-ring)).  While these were not the primary focus of this experiment, they did reveal interesting results with two of the markers screened, and may indicate protrudin involvement in additional cellular pathways.
...
(ref:LC3-cap) A) Representative image of HeLa cells expressing WT protrudin and stained for LC3.  Maximum intensity projection of a confocal stack.  Inset is 20x20 \textmu{}m.  B) Fluorescence profile along the dotted line shown in panel A inset.  Note strong correspondence between protrudin (green) and LC3 (magenta) signal.  C) Protrusion of a HeLa cell expressing WT protrudin and stained for LC3.  Note the lack of LC3 enrichment with protrudin in the tip, compared to the cell body.  Maximum intensity projection of a confocal stack.  D) Representative image of a HeLa cell expressing GFP and stained for LC3.  Note the lack of large, ring-shaped structures as seen in the protrudin expressing cell in panel A.  Maximum intensity projection of a confocal stack.  E) Representative images of neurons expressing mRFP1-tagged LC3 showing range of distribution patterns.  F) Quantification of LC3 distributions in cells co-expressing EGFP-protrudin and mRFP1-LC3.  Single plane spinning disc confocal images.  *n* = 40 cells per condition across two independent batches.  All scale bars are 20 \textmu{}m.
```{r LC3-full-figure, fig.width=16, fig.height=22, fig.path = "../figures/full/", fig.cap="(ref:LC3-cap)", fig.scap="Protrudin effect on LC3 localisation in HeLa cells and cortical neurons"}
LC3_full_plot
```
